...
// Прежняя система сохранений "key=value" вручную - быстро и удобно для демо, но в реальной игре превращается в ад
...
// -=-=-=-=-= Flow =-=-=-=-=-
// Пример:
// Есть радиостанция - она пускает события а слушатели подписываются и получаюи информацию о событиях
// Во Flow есть два главных варианта
// 1. SharedFlow - наше радио событие
// -это как потом радиостанции, трансляции и тп - он существует даже когда никто не слушает и раздаёт события всем подписчикам
...
// -=-=-=-=-= Save через сериализацию =-=-=-=-=- //
// Будем сохранять не строки вручную а объекты целиком
...
import kotlinx.serialization.encodeToString         // Запись на файл
import kotlinx.serialization.decodeFromString       // Чтение с файла
import kotlinx.serialization.json.Json              // Формат фалйа Json

import java.io.File                                 // Для работы с файлами


// -= События игры создаём как раньше но отправлять будем через Flow =-
...
// -= Серверные данные игрока - то что мы хотим сохранять =-
...
class ServerWorld(
    ...
    suspend fun emitEvent(event: GameEvent){
        ...
        // emit - может разослать событие не сразу если подписчики медленные (очередь потоков) [например можно поставить на паузу пока не сможет выполнится]
        // Готовим событие заранее и рассылаем его уже в корутину
    }

    // написать реальную загрузку с файла которая будет записывать загруженную информацию из файла в состояние игрока
    // создать метод server.applyLoaded(playerSave: PlayerSave) который должен:
...
3
            ...
            // подписка 1: слушаем события erver.events
            coroutineScope.launch {
                server.events.collect {event ->
                    // >collect - слушает поток (каждое событие будет попадать в данный слушатель
                    when(event){
            ...
            // после получения состояния     |hp/gold...|
            // нужно в переменные интерфейса ui.///.value
            // так надо поступить с hp, gold, questState
            // [что-то там с присваиванием]
            coroutineScope.launch {
                server.playerState.collect {event ->
                    // нужно достать актуальное состояние
                }
            }
        }
    }
